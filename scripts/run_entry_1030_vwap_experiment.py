#!/usr/bin/env python3
"""Research-only 10:30 entry + intraday VWAP quality experiment.

Uses the 1M/3M 40/60 recommendation set and Rolling 10 lifecycle.

Variants:
  - baseline: original T+1 daily open entry
  - entry_1030: T+1 10:30 bar open entry
  - entry_1030_skip_vwap: T+1 10:30 bar open entry, skip if entry is
    more than 1% above cumulative VWAP available through the 10:30 bar.
...
OUTPUT_DIR = REPO_ROOT / "results" / "entry_1030_vwap_experiment"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run 10:30 entry + VWAP quality experiment.")
    ...
    for current_date in dates:
        ...
        if current_date in entries_by_date:
            ...
            for rec in candidates:
                ...
                entry_price = daily_open if variant.entry_mode == "daily_open" else (intraday or {}).get("entry_open")
                cumulative_vwap = (intraday or {}).get("cumulative_vwap")
                ...
                base_log = {
                    ...
                    "entry_1030_open": (intraday or {}).get("entry_open"),
                    "entry_1030_close": (intraday or {}).get("entry_close"),
                ...
                if entry_price is None or entry_price <= 0:
                    ...
                    continue
                if variant.skip_vwap and poor_entry:
...
def render_report(payload: dict[str, object]) -> str:
    ...
    lines = [
        "# 10:30 Entry + VWAP Quality Experiment",
        "",
        "Research-only experiment. No production scores, recommendations, strategy rules, or database rows were modified.",
        "",
        f"- Entry time tested: {payload['parameters']['entry_time']}",
        f"- VWAP rule: skip when 10:30 entry open > cumulative VWAP through 10:30 + {fmt_pct(payload['parameters']['vwap_threshold'])}",
        "",
        "## Portfolio Metrics",
        "",
...
def main() -> int:
    ...
    variants = [
        ...
        Variant("rolling_10_1m3m_entry_1030_skip_vwap_1pct", "entry_1030", True),
    ]
    results = [
        run_variant(
            variant,
        ...
        summary.append(
            {
                ...
                **result["metrics"],
                ...
                "missing_1030_price_skips": sum(1 for row in entries if row.get("reason") == "missing_entry_price"),
            }
        )
    best = max(summary, key=lambda row: (float(row["sharpe_ratio"]), float(row["cagr"])))
    base = summary[0]
    verdict = (
        f"{best['variant']} is best by Sharpe: {best['sharpe_ratio']:.2f} vs daily-open baseline {base['sharpe_ratio']:.2f}; "
    ...
    payload = {
        ...
        "parameters": {
            ...
            "vwap_threshold": args.vwap_threshold,
            "vwap_timing": "Cumulative intraday VWAP through the 10:30 bar; no later same-day bars are used for the entry-quality decision.",
        },
        "summary": summary,
    ...
    (args.output_dir / "entry_1030_vwap_experiment.json").write_text(json.dumps(payload, indent=2, default=str), encoding="utf-8")
    (args.output_dir / "ENTRY_1030_VWAP_EXPERIMENT.md").write_text(render_report(payload), encoding="utf-8")
    write_csv(args.output_dir / "entry_1030_vwap_summary.csv", summary)
    write_csv(args.output_dir / "entry_1030_vwap_fy_returns.csv", payload["financial_year_returns"])
    write_csv(args.output_dir / "entry_1030_vwap_equity.csv", [row for result in results for row in result["equity_curve"]])
    write_csv(args.output_dir / "entry_1030_vwap_trades.csv", [row for result in results for row in result["trades"]])
    write_csv(args.output_dir / "entry_1030_vwap_entries.csv", [row for result in results for row in result["entry_log"]])
