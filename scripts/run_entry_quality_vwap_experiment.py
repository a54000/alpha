#!/usr/bin/env python3
"""Research-only entry-quality VWAP sizing/skip experiment.

Uses the 1M/3M 40/60 recommendation set and Rolling 10 lifecycle. Entry quality
is judged at the actual entry session:

  - baseline: no VWAP adjustment
  - half-size: if entry open > entry-day VWAP + threshold, allocate half slot
  - skip: if entry open > entry-day VWAP + threshold, skip the entry
...
OUTPUT_DIR = REPO_ROOT / "results" / "entry_quality_vwap_experiment"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Rolling 10 entry-quality VWAP experiment.")
...
def load_vwap(engine, symbols: set[str], start_date: date, end_date: date) -> dict[tuple[str, date], float]:
    query = text(
        """
        SELECT symbol,
    ...
    with engine.connect() as connection:
        rows = connection.execute(query, {"symbols": list(symbols), "start_date": start_date, "end_date": end_date}).mappings().all()
    return {(str(row["symbol"]), row["date"]): float(row["vwap"]) for row in rows if row["vwap"] is not None}
...
def run_variant(
    ...
    prices: dict[str, dict[date, dict[str, float]]],
    vwaps: dict[tuple[str, date], float],
    *,
    start_date: date,
    end_date: date,
    ...
    for current_date in dates:
        ...
        if current_date in entries_by_date:
            ...
            for rec in candidates:
                ...
                entry_vwap = vwaps.get((symbol, current_date))
                entry_vs_vwap = (open_price / entry_vwap - 1.0) if open_price and entry_vwap else None
                poor_entry = entry_vs_vwap is not None and entry_vs_vwap > vwap_threshold
                ...
                if open_price is None or open_price <= 0:
                    ...
                    continue
                if variant.mode == "skip_poor_entry" and poor_entry:
                    entry_log.append({**base_log, "status": "skipped", "reason": "entry_gt_vwap_threshold"})
                    continue
                multiplier = 0.5 if variant.mode == "half_size_poor_entry" and poor_entry else 1.0
...
def render_report(payload: dict[str, object]) -> str:
    ...
    lines = [
        "# Entry Quality VWAP Experiment",
        "",
        "Research-only experiment. No production scores, recommendations, strategy rules, or database rows were modified.",
        "",
        f"- Entry quality threshold: entry open > entry-day VWAP + {fmt_pct(payload['parameters']['vwap_threshold'])}",
        "- Baseline: no entry-quality adjustment",
        "- Half-size: poor-quality entries use half slot allocation",
        "- Skip: poor-quality entries are skipped",
        "",
        "## Portfolio Metrics",
        "",
        "| Variant | CAGR | Total Return | Max DD | Sharpe | Sortino | PF | Win Rate | Trades | Avg Cash | Poor Entries Entered | Poor Entries Skipped |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in rows:
        lines.append(
...
def main() -> int:
    ...
    vwaps = load_vwap(engine, symbols, args.start_date, args.end_date)
    variants = [
        Variant("rolling_10_1m3m_baseline", "baseline"),
        Variant("rolling_10_1m3m_half_size_entry_gt_vwap_1pct", "half_size_poor_entry"),
        Variant("rolling_10_1m3m_skip_entry_gt_vwap_1pct", "skip_poor_entry"),
    ]
    results = [
        run_variant(
            variant,
    ...
    for result in results:
        ...
        poor_entered = [row for row in entries if row.get("status") == "entered" and row.get("poor_entry_quality")]
        poor_skipped = [row for row in entries if row.get("status") == "skipped" and row.get("reason") == "entry_gt_vwap_threshold"]
        summary.append(
            {
                "variant": result["variant"],
                **result["metrics"],
                "entries_entered": sum(1 for row in entries if row.get("status") == "entered"),
                "poor_entries_entered": len(poor_entered),
                "poor_entries_skipped": len(poor_skipped),
            }
        )
    base = summary[0]
    best = max(summary[1:], key=lambda row: (float(row["sharpe_ratio"]), float(row["cagr"])))
    verdict = (
        f"{best['variant']} is the better entry-quality candidate: Sharpe {best['sharpe_ratio']:.2f} vs baseline {base['sharpe_ratio']:.2f}, "
        f"CAGR {best['cagr'] * 100:.2f}% vs baseline {base['cagr'] * 100:.2f}%."
    )
    payload = {
    ...
    (args.output_dir / "entry_quality_vwap_experiment.json").write_text(json.dumps(payload, indent=2, default=str), encoding="utf-8")
    (args.output_dir / "ENTRY_QUALITY_VWAP_EXPERIMENT.md").write_text(render_report(payload), encoding="utf-8")
    write_csv(args.output_dir / "entry_quality_vwap_summary.csv", summary)
    write_csv(args.output_dir / "entry_quality_vwap_fy_returns.csv", payload["financial_year_returns"])
    write_csv(args.output_dir / "entry_quality_vwap_equity.csv", [row for result in results for row in result["equity_curve"]])
    write_csv(args.output_dir / "entry_quality_vwap_trades.csv", [row for result in results for row in result["trades"]])
    write_csv(args.output_dir / "entry_quality_vwap_entries.csv", [row for result in results for row in result["entry_log"]])
